...
module ApplicationHelper
  ...
  def display_human_name(name)
    if !name.try(:text).nil?
      ...
      result = name.try(:prefix).join(', ') unless name.try(:prefix).nil?

      unless name.try(:given).nil?
        result += ' ' if result.length != 0
        result += name.try(:given).join(', ')
      end
      unless name.try(:family).nil?
        result += ' ' if result.length != 0
        result += name.try(:family)
      end

      unless name.try(:suffix).nil?
        result += ' ' if result.length != 0
        result += name.try(:suffix).join(', ')
      end
  ...
  def display_telecom(telecom)
    return if telecom.try(:value).nil?
  ...
  def display_CodeableConcept(codeableConcept)
    result = if codeableConcept.text.present?
               codeableConcept.text
             else
               display_coding(codeableConcept.coding[0])
             end
    sanitize(result)
  end
  ...
  def display_address(address)
    result = if address.present?
               link_to(google_maps(address)) do
                 [address.line.join('<br />'),
                  [[address.city, address.state].join(', '),
                   display_postal_code(address.postalCode)].join(' ')].join('<br />').html_safe
               end
             else
               'None'
             end
  ...
  def display_postal_code(postal_code)
    return 'None' unless postal_code.present?

    sanitize(postal_code.match(/^\d{9}$/) ? postal_code.strip.sub(/([A-Z0-9]+)([A-Z0-9]{4})/, '\1-\2') : postal_code)
  end
  ...
  def display_reference(reference, use_controller: 'default')
    ...
    controller = if use_controller.eql?('default')
                   components.first.underscore.pluralize
                 else
                   use_controller
                 end
  ...
  def display_location_type(list)
    result = if list.empty?
               'None'
             else
               list.map(&:text).join(',<br />')
             end
